...
def start_bot():
    ...
    def handler_call(call):
        ...
        # Добавлен обработчик для банковской карты
        if call.data == 'bank_card':
            bot.edit_message_text(chat_id=chat_id,
                                  message_id=message_id,
                                  text=f'💳 Для пополнения баланса переведите деньги на банковскую карту:\n\n{settings.bank_card_number}\n\nПосле перевода пришлите чек из банка сапорту: @Sheksper_Suport\n\nПосле этого деньги поступят в течение 5-10 минут.\n\n Зайдите на свой профиль в главном меню что бы проверить баланс',
        ...
        # Удалены обработчики для Bitcoin, Qiwi, проверки оплаты и отмены платежа
        ...
        # Добавлен обработчик для поиска пользователя
        if call.data == 'find_user':
            if chat_id == settings.admin_id_1 or chat_id == settings.admin_id_2:
                msg = bot.send_message(chat_id=chat_id,
                                       text='Введите username пользователя (с @ или без) или ID пользователя:')
    ...
    def find_user(message):
        try:
            ...
            if result:
                user_id, username, registration_date, balance = result
                bot.send_message(message.chat.id,
                               f'✅ Пользователь найден:\n\n'
                               f'👤 Username: @{username}\n'
                               f'🆔 ID: {user_id}\n'
                               f'💰 Баланс: {balance} рублей\n'
                               f'📅 Дата регистрации: {registration_date[:19]}',
                               reply_markup=menu.admin_menu)
            else:
                bot.send_message(message.chat.id,
        ...
        try:
            ...
            msg = bot.send_message(chat_id=message.chat.id,
                               text=f'Вы выбрали город {rabota}. \n'
                                    ...
                                    '- Присутствует чат для курьеров, в котором Вы сможете обмениваться опытом с другими сотрудников и просто беседовать на свободные темы\n'
            ...
            if int(message.text) in range(1, int(product.amount_MAX)+1):
                ...
                msg = bot.send_message(chat_id=message.chat.id,
                    text=f'❕ Вы выбрали - {product.product}\n'
                       ...
                       f'Для подтверждения покупки напишите код - {code}')
                ...
                if check == 1:
                    ...
                    bot.send_message(chat_id=message.chat.id,
                                     text=f'✅ Вы успешно купили товар\n\n'
                                          f'Напишите оператору для выдачи адреса @Sheksper_Suport: \n\n'
                                          f'- Перешлите ему это сообщение из бота\n\n'
                                          f'- Отправте чек из банка, что вы перевели боту деньги\n\n'
                                          f'- Напишите адрес, где удобно сдлеать закладку.\n\n'
                                          f'- 30 минут и закладка будет готова! \n\n'
                                          f' Ваш товар: \n\n{list}',
                                     reply_markup=menu.main_menu)
